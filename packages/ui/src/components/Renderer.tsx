...
const useRerenderDependencies = (arg: ReRenderCheckProps) => {
  ...
  // Filter out functions before cloning to prevent DataCloneError
  const optionsWithoutFunctions = Object.fromEntries(
    Object.entries(options).filter(([_, v]) => typeof v !== 'function')
  );
  const _options = cloneDeep(optionsWithoutFunctions);
  if (_options.font) {
