...
import LeftSidebar from './Designer/LeftSidebar.js';
...
const Preview = ({
  ...
  const init = (template: Template, inputOverride?: Record<string, string>) => {
    // Plan C Implementation: Skip getDynamicTemplate to avoid automatic page splitting
    // This ensures elements stay on their designed page, preventing unexpected "1/2" page issues
    // Note: This means dynamic table expansion won't automatically create new pages
    template2SchemasList(template)
      .then(async (sl) => {
        setSchemasList(sl);
  ...
  return (
    <Root size={size} scale={scale}>
      <div style={isHeaderMode ? { display: 'flex', flexDirection: 'column', height: '100%', width: '100%' } : {}}>
        ...
        <div style={{ position: 'relative', flex: 1, display: isHeaderMode ? 'flex' : 'block' }}>
      <CtlBar
        size={size}
        ...
        {...(() => {
          ...
          // In header mode, we have a 48px header. Preview doesn't have rulers.
          const paperBottomY = 48 + paperHeightPx;
          const paperCenterX = size.width / 2;
          return { paperCenterX, paperBottomY };
        })()}
      />
      <UnitPager
        size={size}
      ...
      />
      <div ref={containerRef} style={{ ...size, position: 'relative', overflow: 'auto', flex: 1 }}>
        <Paper
          paperRefs={paperRefs}
          scale={scale}
          size={size}
          ...
          renderSchema={({ schema, index }) => {
            ...
            return (
              <Renderer
                ...
                placeholder={safeContent(schema.content)}
                tabIndex={index + 100}
                onChange={(arg) => {
