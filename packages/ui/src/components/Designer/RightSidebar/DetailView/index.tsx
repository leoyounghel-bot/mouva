...
const DetailView = (props: DetailViewProps) => {
  ...
  const { schemasList, changeSchemas, deselectSchema, activeSchema, pageSize, basePdf, onShowListView, children } = props;
  const form = useForm();
  ...
  useEffect(() => {
    const newWidgets: typeof widgets = {
      ...
      HeadingWidget: (p) => <HeadingWidget {...p} {...props} options={options} />,
    };
  ...
  // Add artText as a type option (it's a special text variant)
  typeOptions.push({ label: 'artText', value: 'text' });
  ...
  if (typeof activePropPanelSchema === 'function') {
    ...
    // Add Heading Level widget for text elements
  const showHeadingWidget = activeSchema.type === 'text' || activeSchema.type === 'multiVariableText' || activeSchema.type === 'artText' || activeSchema.type === 'heading';
  ...
  return (
    <SidebarFrame className={DESIGNER_CLASSNAME + 'detail-view'}>
      ...
      <SidebarBody>
        {/* Heading Widget at Top for Text Elements */}
        {showHeadingWidget && (
          <HeadingWidget
