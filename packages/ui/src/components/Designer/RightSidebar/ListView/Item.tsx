...
import { GripVertical, CircleAlert, LockKeyhole, LockKeyholeOpen } from 'lucide-react';
...
interface Props {
  ...
  /** Data to pass on drag start for external dropping */
  onDragStartData?: any;
  /** Callback to change schema properties */
  onChangeSchemas?: (key: string, value: any) => void;
}
...
const Item = React.memo(
  ...
  React.forwardRef<HTMLLIElement, Props>(function Item(
    ...
    const handleDragStart = (e: React.DragEvent) => {
      console.log('[DEBUG] onDragStart triggered for field:', onDragStartData?.name);
      if (onDragStartData) {
        e.dataTransfer.setData('application/pdfme-field', JSON.stringify(onDragStartData));
        e.dataTransfer.effectAllowed = 'copy';
        
        const dragImage = (e.currentTarget.getAttribute('data-is-handle') === 'true' 
          ? e.currentTarget.parentElement?.cloneNode(true) 
      ...
      >
        ...
        >
          ...
          <Button
            size="small"
            type="text"
            style={{ padding: 0, height: 'auto', marginRight: '0.5rem' }}
            onClick={(e) => {
              e.stopPropagation();
              onChangeSchemas && onChangeSchemas('readOnly', !readOnly);
            }}
            icon={
              readOnly ? (
                <LockKeyhole strokeWidth={1.5} size={16} />
              ) : (
                <LockKeyholeOpen strokeWidth={1.5} size={16} style={{ opacity: 0.3 }} />
              )
            }
          />
