...
export const useInitEvents = ({
  ...
  const initEvents = useCallback(() => {
    const getActiveSchemas = () => {
      ...
      return schemasList[pageCursor].filter((s) => s && ids.includes(s.id));
    };
    ...
    initShortCuts({
      ...
      paste: () => {
        ...
        const elements = pasteSchemas.map((s) => document.getElementById(s.id)).filter((el): el is HTMLElement => el !== null);
      ...
      esc: onEditEnd,
      selectAll: () => {
        const elements = schemasList[pageCursor].map((s) => document.getElementById(s.id)).filter((el): el is HTMLElement => el !== null);
        if (elements.length > 0) {
          onEdit(elements);
        }
      },
