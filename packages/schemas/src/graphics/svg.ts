...
// Helper function to read SVG file as text
const readSvgFile = (input: File | FileList | null): Promise<string> =>
  new Promise((resolve, reject) => {
    ...
    if (file) {
      fileReader.readAsText(file); // Read as text for SVG
    } else {
...
const svgSchema: Plugin<SVGSchema> = {
  ui: (arg) => {
    const { rootElement, value, mode, onChange, stopEditing, tabIndex, theme, schema } = arg;
    ...
    if (value && isValidSVG(value)) {
      ...
      const svgContainerStyle: CSS.Properties = {
        ...fullSize,
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
      };
      Object.assign(svgContainer.style, svgContainerStyle);
    ...
    } else if (value && !isValidSVG(value)) {
      container.appendChild(createErrorElm());
    }

    // Remove button - only show when there is valid content and not default
    ...
    // File input - show when no content or default content, and editable
    if ((!value || isDefault) && editable) {
      ...
      input.addEventListener('change', (event: Event) => {
        ...
        readSvgFile(changeEvent.target.files)
          .then((result) => {
            if (isValidSVG(result)) {
              if (onChange) onChange({ key: 'content', value: result });
            } else {
