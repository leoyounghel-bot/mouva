...
const convertToCellStyle = (styles: Styles | undefined): CellStyle => {
  // Defensive: use default styles if undefined
  const s = styles || defaultStyles;
  return {
    fontName: s.fontName,
    alignment: s.alignment || 'left',
    verticalAlignment: s.verticalAlignment || 'middle',
    fontSize: s.fontSize || 10,
    lineHeight: s.lineHeight || 1,
    characterSpacing: s.characterSpacing || 0,
    backgroundColor: s.backgroundColor || '',
    // ---
    fontColor: s.textColor || '#000000',
    borderColor: s.lineColor || '#000000',
    borderWidth: s.lineWidth || { top: 0.1, right: 0.1, bottom: 0.1, left: 0.1 },
    padding: s.cellPadding || { top: 5, right: 5, bottom: 5, left: 5 },
  };
};
...
const setBorder = (
  ...
  // Defensive: ensure tableStyles exists
  const tableStyles = arg.schema.tableStyles || { borderWidth: 0.1, borderColor: '#000000' };
  div.style[`border${borderPosition}`] = `${String(tableStyles.borderWidth)}mm solid ${
...
export const uiRender = async (arg: UIRenderProps<TableSchema>) => {
  ...
  if (mode === 'designer' && onChange) {
    ...
    // Row height resize handles
    const allRows = showHead ? [table.head[0], ...table.body] : [...table.body];
    let rowOffsetY = 0;
    allRows.forEach((row, rowIndex, rows) => {
      rowOffsetY += row.height;
      // Don't add handle after the last row
      if (rowIndex === rows.length - 1) return;

      const rowDragHandle = document.createElement('div');
      const lineHeight = 5;
      rowDragHandle.style.width = '100%';
      rowDragHandle.style.height = `${lineHeight}px`;
      rowDragHandle.style.backgroundColor = '#eee';
      rowDragHandle.style.opacity = '0.5';
      rowDragHandle.style.cursor = 'row-resize';
      rowDragHandle.style.position = 'absolute';
      rowDragHandle.style.zIndex = '10';
      rowDragHandle.style.left = '0';
      rowDragHandle.style.top = `${rowOffsetY - px2mm(lineHeight) / 2}mm`;

      const setRowColor = (e: MouseEvent) => {
        const handle = e.target as HTMLDivElement;
        handle.style.backgroundColor = '#2196f3';
      };
      const resetRowColor = (e: MouseEvent) => {
        const handle = e.target as HTMLDivElement;
        handle.style.backgroundColor = '#eee';
      };
      rowDragHandle.addEventListener('mouseover', setRowColor);
      rowDragHandle.addEventListener('mouseout', resetRowColor);

      const minRowHeight = 5; // Minimum row height in mm

      rowDragHandle.addEventListener('mousedown', (e) => {
        resetEditingPosition();
        const handle = e.target as HTMLDivElement;
        rowDragHandle.removeEventListener('mouseover', setRowColor);
        rowDragHandle.removeEventListener('mouseout', resetRowColor);
        ...
        const mouseMove = (e: MouseEvent) => {
          ...
          const newRowHeight = currentRowHeight + moveY;
          if (newRowHeight < minRowHeight) {
            newTop = startTop - (currentRowHeight - minRowHeight);
          }
        ...
        const commitRowResize = () => {
          if (move !== 0) {
            const newRowHeight = Math.max(currentRowHeight + move, minRowHeight);
            // Update the row's minCellHeight in the schema based on whether it's head or body
            if (showHead && rowIndex === 0) {
              // Resizing head row - update headStyles padding to achieve new height
              const currentPaddingTop = schema.headStyles.padding?.top ?? 5;
              const currentPaddingBottom = schema.headStyles.padding?.bottom ?? 5;
              const paddingDelta = (newRowHeight - currentRowHeight) / 2;
              onChange([
                {
                  key: 'headStyles',
                  value: {
                    ...schema.headStyles,
                    padding: {
                      ...schema.headStyles.padding,
                      top: Math.max(1, currentPaddingTop + paddingDelta),
                      bottom: Math.max(1, currentPaddingBottom + paddingDelta),
                    },
                  },
                },
              ]);
            } else {
              // Resizing body row - update bodyStyles padding
              const currentPaddingTop = schema.bodyStyles.padding?.top ?? 5;
              const currentPaddingBottom = schema.bodyStyles.padding?.bottom ?? 5;
              const paddingDelta = (newRowHeight - currentRowHeight) / 2;
              onChange([
                {
                  key: 'bodyStyles',
          ...
          rowDragHandle.addEventListener('mouseout', resetRowColor);
          rootElement.removeEventListener('mousemove', mouseMove);
          rootElement.removeEventListener('mouseup', commitRowResize);
        };
        rootElement.addEventListener('mouseup', commitRowResize);
      });
      rootElement.appendChild(rowDragHandle);
    });
  }
