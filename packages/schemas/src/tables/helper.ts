...
export const DEFAULT_TABLE_WIDTH = 150;

export const getScaleFactor = (width: number) => width / DEFAULT_TABLE_WIDTH;
...
export const getDefaultCellStyles = () => ({
  ...
  backgroundColor: '',
  borderColor: '#cbd5e1',
  borderWidth: { top: 0, bottom: 0.2, left: 0, right: 0 },
...
// Helper to extract string content from rich cell objects
const extractCellContent = (cell: unknown): string => {
  if (cell == null) return '';
...
export const getBody = (value: string | string[][]): string[][] => {
  ...
  // Sanitize: ensure all cells are strings, not objects
  if (!Array.isArray(parsed)) return [];
  return parsed.map(row =>
    Array.isArray(row) ? row.map(extractCellContent) : []
  );
};
