...
const createTextDiv = (schema: CellSchema) => {
  // Defensive: ensure borderWidth and padding exist with defaults
  const bw = schema.borderWidth || { top: 0, right: 0, bottom: 0, left: 0 };
  const pd = schema.padding || { top: 5, right: 5, bottom: 5, left: 5 };
...
const cellSchema: Plugin<CellSchema> = {
  ...
  ui: async (arg) => {
    ...
    // Defensive: ensure borderWidth exists
    const borderWidth = schema.borderWidth || { top: 0, right: 0, bottom: 0, left: 0 };
    const { width, height, borderColor, backgroundColor } = schema;
    ...
    const lines = [
      createLineDiv(`${width}mm`, `${borderWidth.top}mm`, '0mm', null, null, '0mm', borderColor || '#000000'),
      createLineDiv(`${width}mm`, `${borderWidth.bottom}mm`, null, null, '0mm', '0mm', borderColor || '#000000'),
      createLineDiv(`${borderWidth.left}mm`, `${height}mm`, '0mm', null, null, '0mm', borderColor || '#000000'),
      createLineDiv(`${borderWidth.right}mm`, `${height}mm`, '0mm', '0mm', null, null, borderColor || '#000000'),
    ];
