...
const getContentHeight = (basePdf: BlankPdf): number => {
  // Validate basePdf properties to avoid NaN
  if (!basePdf || typeof basePdf.height !== 'number' || !Array.isArray(basePdf.padding)) {
    console.warn('[@pdfme/common] getContentHeight: invalid basePdf, using default 297mm (A4)');
    return 297; // A4 height as fallback
  }
  
  const paddingTop = basePdf.padding[0] ?? 0;
  const paddingBottom = basePdf.padding[2] ?? 0;
  const contentHeight = basePdf.height - paddingTop - paddingBottom;
  
  // Ensure we return a valid positive number
  if (!Number.isFinite(contentHeight) || contentHeight <= 0) {
    console.warn('[@pdfme/common] getContentHeight: calculated invalid contentHeight:', contentHeight);
    return basePdf.height > 0 ? basePdf.height : 297;
  }
...
function normalizePageSchemas(
  ...
  // Defensive check: ensure pageSchemas is a valid array
  if (!pageSchemas || !Array.isArray(pageSchemas)) {
    console.warn('[@pdfme/common] normalizePageSchemas: pageSchemas is not an array, returning empty items');
    return { items, orderMap };
  }

  pageSchemas.forEach((schema, index) => {
    // Defensive check: ensure schema has required properties
    if (!schema || !schema.position || typeof schema.height !== 'number') {
      console.warn('[@pdfme/common] normalizePageSchemas: invalid schema at index', index);
...
function placeRowsOnPages(
  ...
  // Defensive checks to prevent "Cannot read properties of undefined (reading 'push')"
  if (!pages || !Array.isArray(pages)) {
    console.warn('[@pdfme/common] placeRowsOnPages: pages is not an array, initializing...');
    // Cannot mutate the passed-in reference, but we can at least avoid crashing
    return startGlobalY;
  }
  
  if (!dynamicHeights || !Array.isArray(dynamicHeights) || dynamicHeights.length === 0) {
    console.warn('[@pdfme/common] placeRowsOnPages: dynamicHeights is invalid, skipping...');
    return startGlobalY;
  }
  
  // Ensure contentHeight is valid to prevent infinite loops
  // Check for NaN, 0, negative, or non-finite values
  if (!contentHeight || contentHeight <= 0 || !Number.isFinite(contentHeight)) {
    console.warn('[@pdfme/common] placeRowsOnPages: contentHeight is invalid:', contentHeight);
    return startGlobalY;
  }
  ...
  if (!Number.isFinite(currentPageIndex) || currentPageIndex < 0) {
    console.warn('[@pdfme/common] placeRowsOnPages: invalid currentPageIndex:', currentPageIndex);
    currentPageIndex = 0;
    currentYInPage = 0;
  }
...
export const getDynamicTemplate = async (
  ...
  for (let pageIndex = 0; pageIndex < template.schemas.length; pageIndex++) {
    let pageSchemas = template.schemas[pageIndex];

    // Defensive check: convert object to array if needed (AI sometimes generates {name: schema} format)
    if (pageSchemas && !Array.isArray(pageSchemas) && typeof pageSchemas === 'object') {
      console.warn('[@pdfme/common] getDynamicTemplate: pageSchemas is object, converting to array');
      pageSchemas = Object.values(pageSchemas) as Schema[];
    }

    // Skip if pageSchemas is still invalid
    if (!pageSchemas || !Array.isArray(pageSchemas) || pageSchemas.length === 0) {
      console.warn('[@pdfme/common] getDynamicTemplate: skipping invalid page at index', pageIndex);
      resultPages.push([]);
      continue;
    }
