...
function TemplatesApp({ isEmbedded }: { isEmbedded: boolean }) {
  ...
  const navigateTo = (name: string, ui: UIType) => {
    if (isEmbedded && window.parent !== window) {
      try {
        // Try to get the parent origin, fallback to '*' if not accessible
        const targetOrigin = window.location.origin !== 'null' ? window.location.origin : '*';
        window.parent.postMessage({ type: 'navigate', payload: { name, ui } }, targetOrigin);
      } catch (e) {
        // If postMessage fails, fall back to direct navigation
        console.warn('postMessage failed, falling back to direct navigation:', e);
