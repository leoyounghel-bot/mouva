    ...
    },
    {
      ...
      content: (
        <select
          ...
          onChange={(e) => {
             if (designer.current) {
                const size = getPageSize(e.target.value);
                const currentTemplate = designer.current.getTemplate();
                const newTemplate = cloneDeep(currentTemplate);
                
                // Check if basePdf is a dictionary object with 'width' property (not a buffer or string)
                if (typeof newTemplate.basePdf === 'object' && newTemplate.basePdf !== null && 'width' in newTemplate.basePdf) {
                  // Cast to any or assertion to help TS if needed, but 'in' check should suffice for narrowing in some versions
                  // safer to cast to the object type we know it is
                  (newTemplate.basePdf as { width: number; height: number }).width = size.width;
                  (newTemplate.basePdf as { width: number; height: number }).height = size.height;
                  designer.current.updateTemplate(newTemplate);
                } else {
                  toast.warning("Cannot resize template with a PDF background file.");
                }
             }
          }}
          defaultValue="A4"
        >
          {Object.keys(pageSizes).map((sizeName) => (
